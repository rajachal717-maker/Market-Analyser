...
if authentication_status == False:
      ...
      with st.chat_message("assistant", avatar="🤖"):
        with st.spinner("Processing analysis..."):
          try:
            ...
            if not api_key:
              st.error(
                  "GROQ_API_KEY is missing. Please configure it in Streamlit"
                  " Cloud Secrets."
              )
            else:
              chat_llm = ChatGroq(
  ...
  with tab2:
    ...
    st.write(
        "Scan live news feeds and macroeconomic indicators across global"
        " sources."
    )
          ...
          try:
            with DDGS() as ddgs:
              ...
              if not results:
                ...
                st.success(
                    f"Retrieved {len(results)} live records for"
                    f" '{search_query}':"
                )
                for i, r in enumerate(results, 1):
                  with st.expander(
                    ...
                    st.markdown(
                        f"**Reference Link:** [Open]({r.get('href', '#')})"
                    )
          except Exception as e:
